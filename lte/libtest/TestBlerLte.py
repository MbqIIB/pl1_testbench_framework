...
def TestBlerLte(testconf_s, testunit_s): 
    ...
    try:   
        ...
        if re.match('[c|C][m|M][w|W500]', testconf_s.testername, re.I| re.M):
            ...
            tester_h=Anritsu(name=testconf_s.testername,ip_addr=testconf_s.testerip, rat=testunit_s.common.rat.lower(), xmlfile_config=xmlfile_cmw500_config)
            insertPause(2)
...
#                                                                         float(csvreport_h.entry_header_s.meas_bler.dlbler))
                                ...
                                # ---------------------------------------------------------------------------
                                if db_check_permission in ["READ_ONLY", "READ_WRITE"]:
...
#        table_snr_h.save()
    ...
    # Update database 
    if db_check_permission in ["READ_WRITE"]:
